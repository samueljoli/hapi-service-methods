...
internals.register = (args, hapi) => (service) => {
    ...
    Toys.forEachAncestorRealm(hapi.realm, (realm) => {
        const s = internals.state(realm);

        if (!s.setup) { // it's not the root plugin
            if (!s.services) { // plugin that has not registered this plugin explicitly
                internals.setInternalState(s);
                s.services[scope] = {
                    ...rootState.services[scope],
                    [service.name]: method,
                };
